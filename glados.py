...
model = whisper.load_model("small.en") #medium is too slow on CPU, small.en is OK, base is too stupid.
...
def record_audio(filename, threshold_multiplier=1.75, window_size=500, silence_duration=1, max_duration=20, rate=16000,
...
wakeWordModel = Model(
	wakeword_models=[os.path.dirname(os.path.abspath(__file__))+"/models/openWakeWord/glados.tflite"],
	vad_threshold=0.5,
)
